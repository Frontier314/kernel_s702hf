...
//#define SMM6260_STAMP_DEBUG
#ifdef SMM6260_STAMP_DEBUG
#define SMM260_STAMP_MAX 80
...
SMM6260_stamp smm6260stamp[SMM260_STAMP_MAX];
int sendstampindex=0;
void setstamp(int index)
{
	int processid = 0;
	if(sendstampindex>SMM260_STAMP_MAX)
		return;
	processid = smp_processor_id();
	smm6260stamp[sendstampindex].processid = processid;
	smm6260stamp[sendstampindex].index=index;
	smm6260stamp[sendstampindex].stamp =(int)(cpu_clock(processid)&0x3fffffff);	
	sendstampindex++;
...
{
	int i =0;
	for(i=0;i<sendstampindex;i++)
		printk("%d,%d,%x\n",smm6260stamp[i].index,
				smm6260stamp[i].processid,
				smm6260stamp[i].stamp);
	sendstampindex = 0;
}
#else
void setstamp(){};
void printstamp(){};
#endif
#define RMNET_ETH_HDR_SIZE (sizeof(struct iphdr))
...
// SMM6260_TTY0  (hardcoded in smm6260_tty.c)
{"TTY0",       0, SMD_TYPE_APPS_MODEM | SMD_KIND_STREAM, 1},
// SMM6260_DATA0 (hardcoded in smm6260_net.c)
#ifdef CONFIG_SMM6260_MODEM_NET
#if (CONFIG_SMM6260_MODEM_MAX_NUM == 3)
{"DATA0",            1, SMD_TYPE_APPS_MODEM | SMD_KIND_PACKET, 1},
{"DATA1",            2, SMD_TYPE_APPS_MODEM | SMD_KIND_PACKET, 1},
{"DATA2",         3, SMD_TYPE_APPS_MODEM | SMD_KIND_PACKET, 1},
#elif (CONFIG_SMM6260_MODEM_MAX_NUM == 2)
{"TTY1",            3, SMD_TYPE_APPS_MODEM | SMD_KIND_STREAM, 1},
{"DATA0",         1, SMD_TYPE_APPS_MODEM | SMD_KIND_PACKET, 1},
{"DATA1",         2, SMD_TYPE_APPS_MODEM | SMD_KIND_PACKET, 1},
...
{"DATA0",         3, SMD_TYPE_APPS_MODEM | SMD_KIND_PACKET, 1},
#endif
#else
// SMM6260_TTY1  (hardcoded in smm6260_tty.c)
{"TTY1",            1, SMD_TYPE_APPS_MODEM | SMD_KIND_STREAM, 1},
{"TTY2",            2, SMD_TYPE_APPS_MODEM | SMD_KIND_STREAM, 1},
{"TTY3",            3, SMD_TYPE_APPS_MODEM | SMD_KIND_STREAM, 1},
#endif

// SMM6260_DATA1 (hardcoded in smm6260_net.c)
//{"DATA1",         3, SMD_TYPE_APPS_MODEM | SMD_KIND_PACKET, 1},
};

static unsigned char smd_ch_allocated[SMD_CHANNELS];
static int smd_alloc_channel(const char *name, uint32_t cid, uint32_t type);

static void smd_channel_probe(void)
{
...
static unsigned ch_read_send_buffer(struct smd_channel *ch, void **ptr)
{
	unsigned head = ch->send->head;
	unsigned tail = ch->send->tail;
	*ptr = (void *) (ch->send_data + tail);
...
static int ch_read_send_packet_header(struct smd_channel *ch, void *_data, int len)
{
	void *ptr;
	unsigned n;
	unsigned char *data = _data;
...
static void update_packet_state(struct smd_channel *ch)
{
	...
	if (smd_stream_read_avail(ch) < RMNET_ETH_HDR_SIZE)
		return;

	memset(&rx_ip_hdr,0, RMNET_ETH_HDR_SIZE);
	r = ch_read_packet_header(ch, &rx_ip_hdr, RMNET_ETH_HDR_SIZE);
	BUG_ON(r !=RMNET_ETH_HDR_SIZE);
...
static void handle_smd_irq(struct smd_channel *ch, void (*notify)(struct smd_channel *))
{
	...
	//haozz spin_lock_irqsave(&ch->read_lock, flags);

	if (ch_is_open(ch)) {
		spin_lock_irqsave(&ch->read_lock, flags);//haozz
		tmp = ch->recv->state;
		if (tmp != ch->last_state)
			smd_state_change(ch, ch->last_state, tmp);
		ch->update_state(ch);
		spin_unlock_irqrestore(&ch->read_lock, flags);//haozz
		ch->notify(ch->priv, SMD_EVENT_DATA);
	}
	if (do_notify)
		notify(ch);
	//haozz spin_unlock_irqrestore(&ch->read_lock, flags);
}
static void smd_rx_work(struct work_struct *work)
{
...
static int smd_alloc_channel(const char *name, uint32_t cid, uint32_t type)
{
	...
	ch->smmch = acm_register(cid, &ch->notify_low_layer, smd_rx_data_cb, smd_write_complete_cb, ch);
	if(!ch->smmch){
		kfree(ch);
		return -1;
	}
...
static int smd_ch_is_available(struct smd_channel *smd_ch)
{
	int ret;
	struct acm_ch *smmch = smd_ch->smmch;

	ret = smd_ch->smmch->is_connected(smmch);
	if(ret)
	{
		return  smmch->is_opened(smmch);
	}
	return 0;
}
static unsigned int smd_get_send_packet_length(struct smd_channel *ch)
{
	struct iphdr rx_ip_hdr;
	int r;
	r = ch_read_send_packet_header(ch, &rx_ip_hdr, RMNET_ETH_HDR_SIZE);
	BUG_ON(r != RMNET_ETH_HDR_SIZE);
	return ntohs(rx_ip_hdr.tot_len);	
}
static void smd_send_work(struct work_struct *work)
{
	...
	int len, maxsize,packet_length;
	char *chunk;
	static int prevmps512=0;
	smmch = ch->smmch;

	for(;;)
	{
		setstamp(3);//acm write 3
		if(smd_ch_is_available(ch) && ch_is_open(ch))
		{
			maxsize =  smmch->get_write_size(smmch);
			chunk = ch->chuck_data;
			...
			if(!prevmps512){
				...
				/*diog.zhao add for only send one packet one time
				 * so solve big MMS can not be send.	
				 */
				if(ch->type & SMD_KIND_PACKET){
					if(ch->type <=RMNET_ETH_HDR_SIZE)
						continue;
					packet_length = smd_get_send_packet_length(ch);
...
#endif
	 		len = smmch->write(smmch, chunk, len);
			if(len<0){
				msleep(10);
			}else{
