 ...
 * Base on msm-rmnet.c
 *
 * Virtual Ethernet Interface for  SMM6260 phone Networking
 *
 *
...
#define NET_CHANNELS	CONFIG_SMM6260_MODEM_MAX_NUM


#define SMM6260_NET_DEFAULT_MTU			1500//65535 haozz

struct smm6260net_private
{
	smd_channel_t *ch;
	struct net_device_stats stats;
	const char *chname;
	struct wake_lock wake_lock;
	int ucount;/*record how many times it open*/
...
static struct workqueue_struct *smm6260net_wq;

static void do_check_active(struct work_struct *work)
{
	struct smm6260net_private *p =
		container_of(work, struct smm6260net_private, work.work);
	...
	if (--p->active_countdown == 0) {
		...
		queue_delayed_work(smm6260net_wq, &p->work,
...
static void smm6260net_early_suspend(struct early_suspend *handler) {
	if (rmnet0) {
		struct smm6260net_private *p = netdev_priv(to_net_dev(rmnet0));
		p->timeout_us = timeout_suspend_us;
	}
	in_suspend = 1;
}

static void smm6260net_late_resume(struct early_suspend *handler) {
	if (rmnet0) {
		struct smm6260net_private *p = netdev_priv(to_net_dev(rmnet0));
...
static struct early_suspend smm6260net_power_suspend = {
	.suspend = smm6260net_early_suspend,
	.resume = smm6260net_late_resume,
};

static int __init smm6260net_late_init(void)
{
	register_early_suspend(&smm6260net_power_suspend);
	return 0;
}

late_initcall(smm6260net_late_init);
#endif

/* Returns 1 if packet caused rmnet to wakeup, 0 otherwise. */
static int smm6260net_cause_wakeup(struct smm6260net_private *p) {
	int ret = 0;
	ktime_t now;
...
static ssize_t wakeups_xmit_show(struct device *d,
				 ...
				 char *buf)
{
	struct smm6260net_private *p = netdev_priv(to_net_dev(d));
	return sprintf(buf, "%lu\n", p->wakeups_xmit);
}
...
static void smm6260_net_notify(void *_dev, unsigned event)
{
...
static int smm6260net_xmit(struct sk_buff *skb, struct net_device *dev)
{
	struct smm6260net_private *p = netdev_priv(dev);
...
#ifdef CONFIG_MSM_RMNET_DEBUG
			p->wakeups_xmit += smm6260net_cause_wakeup(p);
#endif
		}
	}
...
static struct net_device_stats *smm6260net_get_stats(struct net_device *dev)
{
	struct smm6260net_private *p = netdev_priv(dev);
	return &p->stats;
}

static void smm6260net_set_multicast_list(struct net_device *dev)
{
}

static void smm6260net_tx_timeout(struct net_device *dev)
{
	pr_info("smm6260net_tx_timeout()\n");
}

static struct net_device_ops smm6260net_ops = {
	.ndo_open = smm6260net_open,
	.ndo_stop = smm6260net_stop,
	.ndo_start_xmit = smm6260net_xmit,
	.ndo_get_stats = smm6260net_get_stats,
	.ndo_set_multicast_list = smm6260net_set_multicast_list,
	.ndo_tx_timeout = smm6260net_tx_timeout,
};

static void __init smm6260net_setup(struct net_device *dev)
{

	dev->watchdog_timeo = 20*HZ;
	dev->features		= 0;
	dev->netdev_ops		= &smm6260net_ops,
	dev->type		= ARPHRD_NONE;
	dev->flags		= IFF_POINTOPOINT | IFF_NOARP;
	dev->mtu		= SMM6260_NET_DEFAULT_MTU;
...
static int __init smm6260net_init(void)
{
	int ret;
	struct device *d;
...
#ifdef CONFIG_MSM_RMNET_DEBUG
	smm6260net_wq = create_workqueue("rmnet");
#endif

	for (n = 0; n < NET_CHANNELS; n++) {
		dev = alloc_netdev(sizeof(struct smm6260net_private),
				   "rmnet%d", smm6260net_setup);
