...
{
	unsigned int cfg;
	printk("s3cfb_enable_mipi_dsi_mode\n");
	cfg = readl(ctrl->regs + S3C_VIDCON0);
...
{
	u32 reg = 0;
//	s3cfb_enable_mipi_dsi_mode(ctrl, 1);//yan
...
int s3cfb_set_clock(struct s3cfb_global *ctrl)
{
	...
	if (pdata->hw_ver == 0x70) {
		...
		//printk("pdata->clk_name:%s\n", pdata->clk_name);
		//printk(KERN_INFO "1,FIMD src sclk = %d\n", src_clk);
