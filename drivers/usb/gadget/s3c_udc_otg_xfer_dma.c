...
#if defined(CONFIG_BATTERY_SAMSUNG) ||defined(CONFIG_BQ2415X_POWER)
...
#if defined(CONFIG_MACH_SMDKC110) || defined(CONFIG_MACH_SMDKV210) || defined(CONFIG_MACH_STUTTGART)
bool s3c_udc_is_adc_cable()
{
	...
	//msleep(15);
	count ++;
        if ((__raw_readl(dev->regs+S3C_UDC_OTG_HPRT) & 0xc00) == 0xc00)// new identification
		{
		count1++;
		//printk("=====================AC:count=%d=============\n ",count);
		}
