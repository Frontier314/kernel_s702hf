...
static void s3c_udc_soft_disconnect_lenovo(void);
static void s3c_usb_connect_work(struct work_struct *work);
...
void s3c_usb_device_power_control(int is_active)
{
	/*diog.zhao,0614*/
#if 0
	struct s3c_udc *dev =the_controller;
	struct usb_gadget *gadget=NULL;
	static int pre_active = 1;
	if(pre_active == is_active)
		return;
	pre_active = is_active;
	if(the_controller)
		gadget = &(the_controller->gadget);
	if(gadget)
		s3c_vbus_enable(gadget, is_active);
#endif
}
/*
		...
		int (*bind)(struct usb_gadget *))
{
	...
	printk(KERN_INFO "Registered gadget driver '%s'\n",
			driver->driver.name);
	INIT_DELAYED_WORK_DEFERRABLE(&usb_connect_work, s3c_usb_connect_work);
...
#ifdef CONFIG_PM
static bool otg_device_is_suspend = false;
void s3c_otg_device_power_control(int is_active)
{
	struct s3c_udc *dev = the_controller;
	struct usb_gadget *gadget=NULL;

	if(otg_device_is_suspend)
		return;
...
EXPORT_SYMBOL_GPL(s3c_otg_device_power_control);
...
//#endif
	...
	otg_device_is_suspend = true;
	s3c_udc_soft_disconnect_lenovo();
	cancel_delayed_work(&usb_connect_work);
	return 0;
}
...
#endif
		s3c_udc_soft_disconnect_lenovo();
		schedule_delayed_work(&usb_connect_work, msecs_to_jiffies(600));
//#endif

		if (dev->driver->resume)
			dev->driver->resume(&dev->gadget);
	}
	otg_device_is_suspend = false;
